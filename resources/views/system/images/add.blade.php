    ...
    <section class="tm-section tm-section-3">
        <form action="{{ route('images.store') }}" class="tm-contact-form" method="post" enctype="multipart/form-data">
                    ...
                    <div class="col-8">
                        ...
                        <select class="selectpicker" multiple data-live-search="true" name="vips[]" id="Names">
                            ...
                            @if (count($VIPs) > 0)
                                @foreach ($VIPs as $VIP)
                                    <option value="{{ $VIP->id }}"
                                        @if (old('vips') != null) @if (in_array($VIP->id, old('vips')))
                                        selected @endif
            ...
            <div class="form-group mb-4">
                ...
                <input type="text" class="form-control col-md-8 typeahead" value="{{ old('image_occasion') }}"
                    name="image_occasion" id="image_occasion">
            </div>
            <div class="form-group mb-4">
                <label class="col-form-label"><strong>{{ __('Image Quality') }}</strong></label><br />
                <input type="text" class="form-control col-md-8" value="{{ old('image_quality') }}" name="image_quality"
                    id="image_quality">
            </div>
            <div class="form-group mb-4">
                <label class="col-form-label"><strong>{{ __('Image Location') }}</strong></label><br />
                <input type="text" class="form-control col-md-8 typeahead" value="{{ old('image_location') }}"
                    name="image_location" id="image_location">
            </div>
            <div class="form-group mb-4">
                <label class="col-form-label"><strong>{{ __('Image Date') }}</strong></label><br />
                <input type="date" class="form-control col-md-8" name="image_date" value="{{ old('image_date') }}"
                    id="image_date">
            </div>
            <div class="form-group mb-4">
                <div class="row">
                ...
                <div class="row">
                    <div class="col-8">
                        ...
                        <select class="selectpicker" data-live-search="true" name="image_color_type"
                            ...
                            <option value="2" @if (old('image_color_type') == 2) selected @endif>{{ __('Coloured') }}
                            </option>
